...
## 액션
- `web.panel` — 접속 패널(curses 오버레이). 로컬/공개 URL 읽기전용 표시 + **username·password 실시간 편집**. 저장 시 재시작 없이 새 연결부터 적용(URL은 고정). `Ctrl-G` 랜덤 비번 생성.
...
## 테스트
```sh
python3 -m unittest test_config test_ws test_ptybridge test_authstate test_serve
```
