...
| **attention** | 가장 최근 완료(idle/done/blocked)된, 손이 필요한 에이전트로 focus | `prefix+ctrl+l` |
| **tab-history** | 브라우저식 탭 back/forward | `prefix+ctrl+o`/`prefix+ctrl+i` |

## 구조

```
herdr-nav/
  herdr-plugin.toml   # 통합 매니페스트 — events 2, panes 2, actions 16
  ...
  tabhist.py          # 탭 히스토리 (record/back/forward)       + test_tabhist.py
```
...
| `nav.attention-focus` | 최근 완료 에이전트 | `nav.tab-back` / `nav.tab-forward` | 탭 back/forward |
